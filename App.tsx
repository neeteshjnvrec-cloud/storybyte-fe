      ...
      <View style={[styles.homeContainer, { backgroundColor: theme.background }]}>
        ...
        {activeTab === 'home' ? (
          loadingStories ? (
            ...
            <HomeScreen 
            stories={stories}
            setSelectedStory={async (story: any) => {
               // Stop and unload previous audio when switching stories
               if (sound) {
              ...
              {['stats', 'history', 'progress', 'preferences'].map((tab) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    if (tab === 'progress') loadUserProgress();
                  }}
                >
...
const playAudio = async () => {
  try {
    ...
    // Track play when starting audio
    try {
      await ApiService.trackPlay(selectedStory._id || selectedStory.id, 0, 0, false);
    } catch (error) {
...
if (screen === 'detail' && selectedStory) {
  return (
    <StoryDetailScreen 
      story={selectedStory} 
      onBack={async () => {
        // Safe check: stop audio before navigating back
        if (sound) {
          await sound.stopAsync();
          setIsPlaying(false);
        }
        // Reload stories to get updated ratings/plays
        await loadStories();
        setScreen('home');
      }} 
      // This is where pauseAudio is passed to the screen
  ...
  if (screen === 'signup') return <SignupScreen name={name} setName={setName} email={email} setEmail={setEmail} password={password} setPassword={setPassword} loading={loading} onSignup={handleSignup} onLogin={() => setScreen('login')} onGoogleSignIn={() => {}} />;
  if (screen === 'login') return <LoginScreen email={email} setEmail={setEmail} password={password} setPassword={setPassword} loading={loading} onLogin={handleLogin} onSignup={() => setScreen('signup')} onGoogleSignIn={() => {}} onForgot={() => setScreen('forgot')} />;
